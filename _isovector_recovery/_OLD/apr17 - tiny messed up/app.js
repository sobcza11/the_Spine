...
function computeDelta(series, monthsBack) {
  ...
  const pastIndex = currentIndex - monthsBack; 
...
function computeDeltaWithDates(series, dates, monthsBack) {
  if (!Array.isArray(series) || !Array.isArray(dates)) return null;
  ...
  if (bestIdx < 0 || bestIdx >= series.length) return null;

  const current = Number(series[series.length - 1]);
  const past = Number(series[bestIdx]);
...
function renderEquitiesIndustryTable(container, pkg, sortMode, metric) {
  if (!container) return;

  const rows = pkg?.industries_latest || [];
  if (!rows.length) {
    container.innerHTML = `<div class="panel-placeholder">No industry data available.</div>`;
    return;
  }

  const sorted = [...rows].sort((a, b) => {
    const aVal = Number(a.signal ?? a.value ?? 0);
    const bVal = Number(b.signal ?? b.value ?? 0);
    ...
    <div class="equities-industry-table-wrap">
      <table class="equities-industry-table">
        <thead>
          <tr>
            <th>Industry</th>
            <th>${metric === "no" ? "New Orders" : "PMI"}</th>
          </tr>
        </thead>
        <tbody>
          ${sorted.map((r) => {
            const value = Number(r.signal ?? r.value);
            const valueClass = Number.isFinite(value)
              ? (value > 0 ? "equities-cell-positive" : value < 0 ? "equities-cell-negative" : "equities-cell-neutral")
              : "";

            return `
              <tr>
                <td>${String(r.industry ?? "--")}</td>
                <td class="num ${valueClass}">${formatNumber(value, 2)}</td>
              </tr>
            `;
          }).join("")}
        </tbody>
...
async function renderEquities() {
  ...
  const industryMetric = String(equitiesControls.industryMetric?.value || "pmi").toLowerCase();
  const etfFocus = String(equitiesControls.etfFocus?.value || "SPY").toUpperCase();

  const industryBadge = document.getElementById("equities-industry-badge");
    if (industryBadge) {
      industryBadge.textContent = industryMetric === "no"
        ? "New Orders Breadth Signal"
        : "PMI Breadth Signal";
    }
  ...
  const liveUniverse = getEquitiesIndexUniverse();

  updateStatValue(
    document.getElementById("equities-index-focus"),
    regionIsLive ? liveUniverse.join(" / ") : "Coming Soon"
  );
  updateStatValue(
    document.getElementById("equities-index-state"),
    regionIsLive ? "Loading" : "Coming Soon"
  );
  updateStatValue(
    document.getElementById("equities-index-mode"),
    regionIsLive ? "USA Multi-Index" : "Unavailable"
  );
  ...
  updateStatValue(document.getElementById("equities-industry-state"), "Loading");

  updateStatValue(document.getElementById("equities-sigma-selected"), etfFocus);
...
const selectedIndustryHealth =
  industryMetric === "no" ? equitiesData.health.no : equitiesData.health.pmi;

const selectedIndustryLabel =
  industryMetric === "no" ? "New Orders" : "PMI Composite";

if (!selectedIndustryHealth) {
  renderEquitiesIndexPlaceholder(
    "equities-industry-chart",
    `${selectedIndustryLabel} endpoint unavailable.`
  );
  updateStatValue(document.getElementById("equities-industry-pmi"), "--");
  updateStatValue(document.getElementById("equities-industry-bias"), "--");
  ...
  if (isSectorEtf(etfFocus)) {
    const metricPayload = industryMetric === "no" ? equitiesData.no : equitiesData.pmi;
    const metricHealth = industryMetric === "no" ? equitiesData.health.no : equitiesData.health.pmi;
    const metricLabel = industryMetric === "no" ? "New Orders" : "PMI Composite";
    const metricPkg = metricHealth
      ? getEquitiesPmiPackageForEtf(metricPayload, etfFocus)
      : null;
    const overall = metricPkg?.latest?.overall || null;

    if (industrySubtitle) {
      industrySubtitle.textContent = `${metricLabel} — ${etfFocus} | ${getEquitiesTickerLabel(etfFocus)}`;
    }

    if (!metricHealth) {
      renderEquitiesIndexPlaceholder("equities-industry-chart", `${metricLabel} endpoint unavailable.`);
      updateStatValue(document.getElementById("equities-industry-pmi"), "--");
      updateStatValue(document.getElementById("equities-industry-bias"), "--");
      updateStatValue(document.getElementById("equities-industry-state"), "Endpoint Unavailable");
    } else if (!metricPkg || !overall) {
      renderEquitiesIndexPlaceholder("equities-industry-chart", `No ${metricLabel} available for ${etfFocus}.`);
      updateStatValue(document.getElementById("equities-industry-pmi"), "--");
      updateStatValue(document.getElementById("equities-industry-bias"), "--");
...
const biasText = String(overall?.bias ?? "--").toLowerCase();
      ...
      const currentIndustrySort = equitiesControls.industrySort?.value || "sig";
      ...
      const latestDate =
        metricPkg.meta?.latest_date ||
        metricPkg.industries_latest?.[0]?.date ||
        "--";
      ...
      if (industrySource) {
        industrySource.innerHTML = `Source: Monthly ISM ${industryMetric === "no" ? "New Orders" : "PMI"} + ETF Proxy Mapping (OC) | Manually Curated | Showing ${formatMonthYearLabel(latestDate)}`;
      }
    }
...
updateStatValue(
  ...
  document.addEventListener("click", (event) => {
    const trigger = event.target.closest("[data-view], .nav-parent[data-parent]");
    if (!trigger) return;

    if (trigger.disabled) return;
    if (trigger.dataset.locked === "true") return;

    const parentKey = trigger.dataset.parent;
