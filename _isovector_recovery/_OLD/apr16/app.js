...
function buildIndustryDeltaLookup(etfPkg) {
  ...
  if (!etfPkg || !Array.isArray(etfPkg.industries_latest)) return lookup;

  const history = Array.isArray(etfPkg.industries_history) ? etfPkg.industries_history : [];

  if (!history.length) {
    etfPkg.industries_latest.forEach((row) => {
      const key = `${row.pmi_type}||${row.industry}`;
      lookup[key] = { delta3m: null, delta6m: null };
    });
    return lookup;
  }
  ...
  Object.entries(grouped).forEach(([key, rows]) => {
    ...
    const series = rows.map((x) => x.pmi);

    lookup[key] = {
      delta3m: computeDelta(series, 3),
...
function renderEquitiesIndustryTable(container, etfPkg) {
  if (!container) return;

  if (!etfPkg || !Array.isArray(etfPkg.industries_latest) || !etfPkg.industries_latest.length) {
    container.innerHTML = `<div class="panel-placeholder">No mapped PMI industry detail available.</div>`;
    return;
  }

const deltaLookup = buildIndustryDeltaLookup(etfPkg);

const enrichedItems = etfPkg.industries_latest.map((row) => {
  const key = `${row.pmi_type}||${row.industry}`;
  const deltaInfo = deltaLookup[key] || {};

  return {
    ...row,
    delta3m: deltaInfo.delta3m ?? null,
    delta6m: deltaInfo.delta6m ?? null
  };
});

  const rank3m = buildRankBuckets(enrichedItems, "delta3m");
  const rank6m = buildRankBuckets(enrichedItems, "delta6m");

  const cellClass = (key, value, topSet, bottomSet) => {
    if (topSet.has(key)) return "equities-cell-rank-top";
    if (bottomSet.has(key)) return "equities-cell-rank-bottom";
    if (Number.isFinite(Number(value)) && Number(value) < 0) return "equities-cell-negative";
    return "";
  };

  const sections = ["manu", "nonmanu"].map((pmiType) => {
    const rows = enrichedItems
      .filter((row) => row.pmi_type === pmiType)
    ...
    const body = rows.map((row) => {
      const key = `${row.pmi_type}||${row.industry}`;
      const cls3m = cellClass(key, row.delta3m, rank3m.top, rank3m.bottom);
      const cls6m = cellClass(key, row.delta6m, rank6m.top, rank6m.bottom);

      return `
        <tr>
          <td>${row.industry}</td>
          <td class="num">${Number(row.pmi).toFixed(1)}</td>
          <td class="num ${cls3m}">${formatDelta(row.delta3m)}</td>
          <td class="num ${cls6m}">${formatDelta(row.delta6m)}</td>
          <td class="num">${row.signal >= 0 ? "+" : ""}${Number(row.signal).toFixed(1)}</td>
          <td class="num">${Number(row.weight).toFixed(1)}</td>
        </tr>
      `;
    }).join("");

    const label = pmiType === "manu" ? "Manufacturing" : "Non-Manufacturing";
  ...
  const overall = etfPkg.latest?.overall || {};
  const manu = etfPkg.latest?.manu || {};
  const nonmanu = etfPkg.latest?.nonmanu || {};
...
async function loadEquitiesData() {
  ...
  const [indexRes, pmiRes, breadthRes, sigmaRes] = results;
  ...
  try {
    ...
    if (!equitiesData.health.pmi) {
      ...
      if (isSectorEtf(etfFocus)) {
        ...
        if (industrySubtitle) {
  industrySubtitle.textContent = `PMI Composite — ${etfFocus} | ${getEquitiesTickerLabel(etfFocus)}`;
}
        ...
        if (!equitiesData.health.pmi) {
          ...
          updateStatValue(document.getElementById("equities-industry-bias"), "--");
          updateStatValue(document.getElementById("equities-industry-state"), "Endpoint Unavailable");
        } else if (!pmiPkg || !overall) {
          renderEquitiesIndexPlaceholder("equities-industry-chart", `No PMI Composite available for ${etfFocus}.`);
          updateStatValue(document.getElementById("equities-industry-pmi"), "--");
          updateStatValue(document.getElementById("equities-industry-bias"), "--");
          ...
          const latestDate = pmiPkg.meta?.latest_date || pmiPkg.industries_latest?.[0]?.date || "--";
          ...
          if (industrySource) {
            industrySource.innerHTML = `Source: Monthly ISM PMI + ETF Proxy Mapping | the_Spine | Showing ${formatMonthYearLabel(latestDate)}`;
          }
        }
