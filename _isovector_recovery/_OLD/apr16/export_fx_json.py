...
# ---------------------------------------------------------
# Make src-layout imports work when running:
# python .\export_fx_json.py
# from the repo root.
# ---------------------------------------------------------
...
from spine.core.universe.fx_alignment import align_fx_universe
from spine.core.universe.fx_pairs import get_fx_universe

# ---------------------------------------------------------
# Governed inputs
# ---------------------------------------------------------
PRICE_PARQUET = REPO_ROOT / "data" / "spine_us" / "us_fx_spot_cross_t2.parquet"
SPREADS_PARQUET = REPO_ROOT / "data" / "spine_us" / "us_fx_10y_spreads.parquet"

# ---------------------------------------------------------
# Export targets
# ---------------------------------------------------------
PRICE_JSON = REPO_ROOT / "fx_price_data.json"
SPREADS_JSON = REPO_ROOT / "fx_spreads_data.json"
SIGMA_JSON = REPO_ROOT / "fx_sigma_data.json"
UNIVERSE_JSON = REPO_ROOT / "fx_universe.json"
...
# LOCK UNIVERSE
# =========================================================
registry = get_fx_universe()
price_symbols = sorted(df_price["symbol"].dropna().unique().tolist())
spread_pairs = sorted(df_spreads["pair"].dropna().unique().tolist())

COMMON_FX = align_fx_universe(price_symbols, spread_pairs, registry)

if not COMMON_FX:
    raise ValueError("FX universe collapsed to zero — alignment failure")

missing_registry = sorted(set(registry) - set(COMMON_FX))
if missing_registry:
    print("WARNING: Missing registry pairs:", missing_registry)

print("FX Universe Locked:", COMMON_FX)


# =========================================================
# PRICE EXPORT
# =========================================================
price_payload: dict[str, list[dict[str, object]]] = {}

for symbol, g in (
    df_price[df_price["symbol"].isin(COMMON_FX)]
...
if not price_payload:
    raise ValueError("PRICE export produced zero aligned symbols.")
...
# =========================================================
# SPREADS EXPORT
# Comparative Spread & Rate Differential are the same
# so export one governed field: yld_10y_diff
# =========================================================
spread_payload: dict[str, list[dict[str, object]]] = {}

for pair, g in (
    df_spreads[df_spreads["pair"].isin(COMMON_FX)]
...
if not spread_payload:
    raise ValueError("SPREAD export produced zero aligned pairs.")
...
# =========================================================
# SIGMA EXPORT
# Derived only from the same aligned universe
# =========================================================
...
# =========================================================
# UNIVERSE EXPORT
# Optional but recommended for frontend authority
# =========================================================
write_json(UNIVERSE_JSON, COMMON_FX)
...
print("locked fx universe:", COMMON_FX)

required_core = {"AUDUSD", "EURUSD", "GBPUSD", "USDCAD", "USDCHF", "USDJPY"}
