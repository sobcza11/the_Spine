...
from US_TeaPlant.bridges.macro_cpi_bridge import build_macro_cpi_canonical
...
# scripts/build_macro_cpi_canonical.py

"""
scripts/build_macro_cpi_canonical.py

Orchestrates the macro CPI build for the_Spine:

Usage (from repo root):

    $env:PYTHONPATH = "$PWD\\src"
    python scripts/build_macro_cpi_canonical.py
"""
...
def main() -> int:
    ...
    print("[SCRIPT-MACRO-CPI] Building canonical CPI leaf …")
    df_cpi = build_macro_cpi_canonical(start_date=start_date, end_date=end_date)
    print(
        f"[SCRIPT-MACRO-CPI] CPI canonical done: rows={len(df_cpi):,}, "
        f"ccy={df_cpi['ccy'].nunique() if not df_cpi.empty else 0}"
    )
