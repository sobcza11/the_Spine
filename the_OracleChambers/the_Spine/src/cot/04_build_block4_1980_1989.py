"""
Block 4 (1980–1989) — Controlled Legacy COT Extension
...
Usage (PowerShell example):
  ...
  python src/cot/04_build_block4_1980_1989.py --raw_dir data/cot/raw/block4_1980_1989 --merge_into_store
"""
...
def _standardize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for alias_in, alias_out in COL_ALIASES.items():
        key = alias_in.lower().strip()
        if key in lower_map and alias_out not in df.columns:
            df = df.rename(columns={lower_map[key]: alias_out})
...
def _filter_era(df: pd.DataFrame, start: str = "1980-01-01", end: str = "1990-01-01") -> pd.DataFrame:
...
def _read_csv_any(filepath: str) -> pd.DataFrame:
    # handle odd delimiters; try common options
    try:
...
def build_block4(raw_dir: str, out_dir: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for fp in files:
        ...
        df = _filter_era(df, "1980-01-01", "1990-01-01")
        if df.empty:
            continue
    ...
    weekly = _filter_era(weekly, "1980-01-01", "1990-01-01")
    ...
    weekly_out = os.path.join(out_dir, "cot_block4_1980_1989_weekly_std.parquet")
    monthly_out = os.path.join(out_dir, "cot_block4_1980_1989_monthly.parquet")
...
def main() -> None:
    ...
    ap.add_argument("--raw_dir", type=str, required=True, help="Directory with legacy COT CSVs for 1980–1989.")
