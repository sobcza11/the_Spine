"""
FX Spot Canonical (T2) — HIST builder (Tiingo)

Tier-2 pairs:
USDCAD, AUDUSD, NZDUSD, EURJPY, AUDJPY, USDSEK, USDNOK, USDMXN, USDZAR
...
Optional:
- FX_START_DATE (default 2020-01-01; floored)
...
def main() -> int:
    ...
    if not frames:
        raise RuntimeError("No FX data returned for T2 universe; refusing to write empty canonical leaf.")

    df = pd.concat(frames, ignore_index=True)
    df = df.sort_values(["symbol", "date"]).drop_duplicates(["symbol", "date"], keep="last").reset_index(drop=True)

    _upload_parquet_to_r2(df, R2_FX_SPOT_T2_KEY)

    print("\nFX T2 HIST build complete")
    print("rows:", len(df))
