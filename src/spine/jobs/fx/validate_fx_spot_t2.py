"""
FX Spot Canonical (T2) — Validator
"""
...
R2_KEY_CANONICAL = "spine_us/us_fx_spot_canonical_t2.parquet"
...
def main() -> int:
    ...
    want = set([s.upper() for s in T2_PAIRS])
    miss = sorted(list(want - have))
    extra = sorted(list(have - want))
    if miss:
        raise RuntimeError(f"Canonical leaf missing expected T2 symbols: {miss}")
    if extra:
        raise RuntimeError(f"Canonical leaf contains unexpected symbols (not in T2): {extra}")
