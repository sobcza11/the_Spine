...
def main():
    ...
    usd_cad = fx_payload.setdefault("pairs", {}).setdefault("USD/CAD", {})

    if usd_cad.get("metric") and usd_cad.get("rows"):
        existing_metric = usd_cad["metric"]
        usd_cad.setdefault("metrics", {})
        usd_cad["metrics"][existing_metric] = {
            "metric": existing_metric,
            "latest": usd_cad.get("latest"),
            "rows": usd_cad.get("rows", []),
            "source": usd_cad.get("source"),
            "method": usd_cad.get("method"),
            "as_of_date": usd_cad.get("as_of_date"),
        }
    ...
    usd_cad["metrics"]["WTI Inv."] = {
        "metric": "WTI Inv.",
        "source": wti_payload["source"],
        "method": wti_payload["method"],
        "as_of_date": wti_payload["as_of_date"],
        "latest": wti_rows[-1] if wti_rows else None,
        "rows": wti_rows
    }

    usd_cad["metric"] = usd_cad.get("metric") or "US-CA 2Y"
    usd_cad["active_metric"] = "WTI Inv."
