...
def main() -> None:
    ...
    if df[["symbol", "date", "value"]].isna().any().any():
        raise ValueError("Nulls found in canonical daily rates leaf")

    dupes = int(df.duplicated(subset=["symbol", "date"]).sum())
    if dupes > 0:
        raise ValueError(f"Duplicate (symbol,date) rows found: {dupes}")

    expected_symbols = set(DAILY_RATES_SERIES.keys())
    found_symbols = set(df["symbol"].unique())
    missing_symbols = expected_symbols - found_symbols
    if missing_symbols:
        raise ValueError(f"Missing expected daily symbols: {sorted(missing_symbols)}")

    last_date = df["date"].max()
    lag_days = (pd.Timestamp.utcnow().tz_localize(None) - last_date).days
    if lag_days > DAILY_RATES_MAX_LAG_DAYS:
        raise ValueError(
            f"daily rates freshness failed. last_date={last_date.date()} "
            f"lag_days={lag_days} allowed={DAILY_RATES_MAX_LAG_DAYS}"
        )

    print("✅ validate_rates_yields_daily_t1 PASSED")
