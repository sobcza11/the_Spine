...
def validate_boc_combined_canonical_v1():
    ...
    if len(df) == 0:
        raise ValueError("[FAIL] No BoC combined canonical rows found")

    print(f"[PASS] BoC combined canonical validation passed ({len(df)} rows)")


if __name__ == "__main__":
    validate_boc_combined_canonical_v1()
