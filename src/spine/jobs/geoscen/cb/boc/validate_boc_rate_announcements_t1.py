...
def validate_boc_rate_announcements():
    df = pd.read_parquet(RATE_ANNOUNCEMENTS_OUTPUT).copy()
    ...
    if len(df) == 0:
        raise ValueError("[FAIL] No BoC rate announcement rows found")

    print(f"[PASS] BoC rate announcements validation passed ({len(df)} rows)")


if __name__ == "__main__":
    validate_boc_rate_announcements()
