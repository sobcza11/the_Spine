...
R2_KEY = f"{R2_PREFIX}/boe_policy_minutes_t1.parquet"


def upload_boe_policy_minutes_to_r2():
    required_env = [
        "R2_ACCESS_KEY_ID",
    ...
    s3.upload_file(
        Filename=str(POLICY_MINUTES_OUTPUT),
        Bucket=os.getenv("R2_BUCKET_NAME"),
        Key=R2_KEY,
    )

    print("[OK] Uploaded BoE policy minutes to R2")
    print(f"[R2_KEY] {R2_KEY}")


if __name__ == "__main__":
    upload_boe_policy_minutes_to_r2()
