...
def validate_boj_combined_canonical_v1():
    ...
    if len(df) == 0:
        raise ValueError("[FAIL] No BoJ combined canonical rows found")

    print(f"[PASS] BoJ combined canonical validation passed ({len(df)} rows)")


if __name__ == "__main__":
    validate_boj_combined_canonical_v1()
