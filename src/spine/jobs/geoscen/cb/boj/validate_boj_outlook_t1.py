...
def validate_boj_outlook():
    df = pd.read_parquet(OUTLOOK_OUTPUT).copy()
    ...
    if len(df) == 0:
        raise ValueError("[FAIL] No BoJ Outlook rows found")

    print(f"[PASS] BoJ Outlook validation passed ({len(df)} rows)")


if __name__ == "__main__":
    validate_boj_outlook()
