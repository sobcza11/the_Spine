"""
Ingest Beige Book archive documents into GeoScen canonical text format — v1.

Run:
python -m spine.jobs.geoscen.beige_book.ingest_beige_book_t1
"""
...
def _get_html(url: str) -> str:
    response = requests.get(
        url,
        timeout=30,
        headers={"User-Agent": "the_Spine GeoScen Beige Book v1"},
    )
...
def _parse_date_from_url_or_text(url: str, text: str, year: int) -> str | None:
    ...
    match = re.search(r"beigebook(\d{4})(\d{2})(\d{2})", url_l)
    if match:
...
def _extract_document_links(year: int, year_url: str) -> list[str]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if "beigebook" not in url_l:
            continue

        if "default" in url_l or "archive" in url_l:
            continue

        if not url_l.endswith((".htm", ".html")):
            continue

        if url_l.endswith("-summary.htm"):
            full_url = url.replace("-summary.htm", ".htm")
            links.add(full_url)
        else:
            links.add(url)
...
def main() -> None:
    ...
    if not year_pages:
        raise RuntimeError("No Beige Book year pages discovered.")
    ...
    for year, year_url in year_pages.items():
        ...
        links = _extract_document_links(year, year_url)
        print(f"[GeoScen:BeigeBook] year={year} links={len(links)}")
        ...
        for url in links:
            ...
            document_id = f"beige_book_{date}_{text_hash[:12]}"
    ...
    if not rows:
        raise RuntimeError("No Beige Book rows parsed.")
    ...
    print(f"[GeoScen:BeigeBook] min_date={df['date'].min().date()}")
    print(f"[GeoScen:BeigeBook] max_date={df['date'].max().date()}")
    print(f"[GeoScen:BeigeBook] wrote={output_path}")
