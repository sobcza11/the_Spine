...
def main():
    ...
    cot_cross_asset = read_csv(
        repo_root / "data/cot/COT_VinV_indexes_1989_2025_monthly.csv"
    )

    btc_cot = read_parquet(
        repo_root / "data/serving/cot/btc_futures_cot_serving_v1.parquet"
    )

    wti = read_parquet(repo_root / "data/wti/wti_inflation_pressure.parquet")
    credit = read_parquet(repo_root / "data/serving/credit/credit_serving_v1.parquet")
    flows = read_parquet(repo_root / "data/serving/flows/fund_flow_serving_v1.parquet")
    ...
    # Existing cross-asset crypto COT proxy.
    cot_crypto_proxy_pressure = latest_value(cot_cross_asset, "Crypto_Index")

    # New BTC futures COT institutional component.
    # Uses net_position_oi_pct until enough history exists for exhaustion_score.
    btc_cot_net_oi = latest_value(btc_cot, "net_position_oi_pct")
    btc_cot_exhaustion = latest_value(btc_cot, "exhaustion_score")

    # Keep BTC pressure bounded & conservative.
    btc_cot_pressure = btc_cot_net_oi
    ...
    components = {
        ...
        "cot_crypto_proxy_pressure": cot_crypto_proxy_pressure,
        "btc_cot_pressure": btc_cot_pressure,
    ...
    rbl_oc = (
        ...
        f"FX={fx_pressure:.3f}, rates={rates_pressure:.3f}, macro={macro_pressure:.3f}, "
        f"equity={equity_pressure:.3f}, COT crypto proxy={cot_crypto_proxy_pressure:.3f}, "
        f"BTC futures COT={btc_cot_pressure:.3f}, commodity={commodity_pressure:.3f}, "
        f"credit={credit_pressure:.3f}, flows={fund_flow_pressure:.3f}. "
        f"Overall: C_FLOW indicates {c_flow_state}. "
        f"BTC futures COT is active, but exhaustion remains history-limited until multiple weekly observations exist."
    )
    ...
    out = pd.DataFrame([{
        ...
        "rbl_oc": rbl_oc,
        **components,
        "btc_cot_net_oi": btc_cot_net_oi,
        "btc_cot_exhaustion": btc_cot_exhaustion,
    }])

    out_path = repo_root / "data/serving/c_flow/c_flow_serving_v4.parquet"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out.to_parquet(out_path, index=False)

    print("OK | C_FLOW serving v4")
