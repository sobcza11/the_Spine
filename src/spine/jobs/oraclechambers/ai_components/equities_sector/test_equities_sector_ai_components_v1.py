...
def main() -> None:
    ...
    if not zt.get("ai_component_ready"):
        failures.append("equities_sector_zt_not_ready")

    if not rbl.get("ai_component_ready"):
        failures.append("equities_sector_rbl_not_ready")

    if zt.get("online_transition_allowed"):
        failures.append("equities_sector_zt_online_gate_open")

    if rbl.get("online_transition_allowed"):
        failures.append("equities_sector_rbl_online_gate_open")

    if rbl.get("source_artifact") != zt.get("artifact"):
        failures.append("rbl_source_artifact_mismatch")

    if zt.get("plane") != "EQUITIES_SECTOR":
        failures.append("zt_plane_invalid")

    if rbl.get("plane") != "EQUITIES_SECTOR":
        failures.append("rbl_plane_invalid")

    result: dict[str, Any] = {
        "artifact": "test_equities_sector_ai_components_v1",
