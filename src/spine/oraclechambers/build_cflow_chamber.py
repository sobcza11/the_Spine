...
def get_score(payload):
    ...
    return latest.get("score") or latest.get("cflow_score") or latest.get("value") or 0
...
def main():
    ...
    scores = {name: float(get_score(payload)) for name, payload in payloads.items()}
    ...
    out = {
        ...
        "observation": f"C•FLOW currently classifies as {state} with a diagnostic score of {score}.",
        "measurement": [
            {
                "component": name,
        ...
        "diagnosis": f"{ranked[0][0]} is the largest active C•FLOW contributor.",
        "attribution": {
            "drivers": [
                {"component": name, "score": round(component_score, 2), "weight": WEIGHTS[name]}
