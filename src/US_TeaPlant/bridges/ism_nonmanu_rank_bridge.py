...
R2_KEY = "spine_us/us_ism_nonmanu_pmi_rank_by_industry_canonical.parquet"
...
def _reshape(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    df["leaf_name"] = "us_ism_nonmanu_pmi_rank_by_industry_canonical"
    ...
    return df[
        [
            ...
            "sector_name", "pmi_rank",
            "leaf_group", "leaf_name", "source_system", "updated_at",
        ]
    ].sort_values(["as_of_date", "sector_name"]).reset_index(drop=True)


def build_us_ism_nonmanu_pmi_rank_by_industry_canonical(
    excel_path: str,
    sheet_name: str = "serv_rank",
) -> pd.DataFrame:
    print(f"[ISM-NONMANU-RANK] Loading {sheet_name} from {excel_path} …")
    df_raw = _load_excel(excel_path, sheet_name)

    print("[ISM-NONMANU-RANK] Reshaping to canonical …")
    df_final = _reshape(df_raw)

    write_parquet_to_r2(df_final, R2_KEY, index=False)

    print(
        f"[ISM-NONMANU-RANK] Wrote canonical Services PMI-rank-by-industry to R2 "
