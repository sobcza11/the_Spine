...
"""
US_TeaPlant.bridges.ism_manu_no_bridge

Builds the canonical US ISM Manufacturing New Orders-by-industry leaf
for the_Spine from the Excel workbook ism_pmi_transp.xlsx (sheet: manu_no).

R2 key: spine_us/us_ism_manu_no_by_industry_canonical.parquet
...
Schema:
    ...
    ccy             string  ("USD")
    sector_name     string
    new_orders_value float64
    leaf_group      string  ("MACRO_ISM")
    leaf_name       string  ("us_ism_manu_no_by_industry_canonical")
...
R2_ISM_MANU_NO_KEY = "spine_us/us_ism_manu_no_by_industry_canonical.parquet"
...
def _reshape_manu_no_to_canonical(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    df_long = df_raw.melt(
        ...
        value_name="new_orders_value",
    )

    df_long = df_long[df_long["new_orders_value"].notna()].copy()
    ...
    df_long["leaf_name"] = "us_ism_manu_no_by_industry_canonical"
...
def build_us_ism_manu_no_by_industry_canonical(
    ...
    df_raw = _load_manu_no_excel(excel_path=excel_path, sheet_name=sheet_name)

    print(
        f"[ISM-MANU-NO] Reshaping manu_no to canonical form "
        f"(rows={len(df_raw)}) …"
    )
    ...
    print(
        ...
        f"to R2 at {R2_ISM_MANU_NO_KEY} (rows={len(df_canonical)})"
    )
