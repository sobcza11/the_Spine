...
R2_KEY = "spine_us/us_ism_nonmanu_no_by_industry_canonical.parquet"
...
def _reshape(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df[df["new_orders_value"].notna()].copy()
    ...
    df["leaf_name"] = "us_ism_nonmanu_no_by_industry_canonical"
    ...
    return df[
        [
            ...
            "sector_name", "new_orders_value",
            "leaf_group", "leaf_name", "source_system", "updated_at",
        ]
    ].sort_values(["as_of_date", "sector_name"]).reset_index(drop=True)


def build_us_ism_nonmanu_no_by_industry_canonical(
    excel_path: str,
    sheet_name: str = "serv_no",
) -> pd.DataFrame:
    print(f"[ISM-NONMANU-NO] Loading {sheet_name} from {excel_path} …")
